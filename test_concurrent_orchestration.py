...
# For better iteration visibility
...
class VerboseMultiAgentOrchestratorConcurrent(MultiAgentOrchestratorConcurrent):
    def orchestrate(self, thread_id, user_message, stream_callback=None, **kwargs):
        ...
        # Define a streaming callback wrapper to log iteration progress
        original_callback = stream_callback
        
        def iteration_tracking_callback(chunk):
            if "[Processing next step...]" in chunk:
                ITERATION_COUNTERS["current"] += 1
                log(f"🔄 ITERATION {ITERATION_COUNTERS['current']} STARTING...", 0)
            
            if original_callback:
                original_callback(chunk)
        
        result = super().orchestrate(thread_id, user_message, stream_callback=iteration_tracking_callback, **kwargs)
        
        log(f"🔄 Orchestration complete with {ITERATION_COUNTERS['current']} iterations", 1)
        return result
...
def setup_test():
    ...
    classifier_config = AzureOpenAIAgentConfig(
        ...
        Regardless of iteration, you should select the most relevant specialists for each type of request.
        If it's a follow-up question based on previous findings, select agents who should investigate further.
    ...
    aws_agent_configs = [
        ("ec2_agent", "EC2 instance analyzer and optimizer", 
         ...
         IMPORTANT: Your analysis should follow a multi-step process:
         - First iteration: Identify immediate issues and suspicious resources
         - Second iteration: Provide detailed recommendations and actions to take
         - Third iteration: If needed, outline a longer-term strategy
         
         At the end of your response, if more investigation is needed, ALWAYS include one of these phrases:
         - "NEXT_STEP: We should now investigate [specific aspect]" to continue the investigation
         - "CONTINUE: Additional analysis needed" for follow-up without specific direction
        ...
        ("billing_agent", "AWS cost analyzer and budget specialist", 
         ...
         IMPORTANT: Your analysis should follow a multi-step process:
         - First iteration: Identify the cost spike sources and immediate concerns
         - Second iteration: Quantify the financial impact and timeline
         - Third iteration: Provide detailed cost containment recommendations
         
         At the end of your response, if more detailed analysis is needed, ALWAYS include one of these phrases:
         - "NEXT_STEP: We should now calculate [specific financial aspect]" to continue
         - "CONTINUE: Additional cost analysis required" for general follow-up
         
         If you identify suspicious spending that suggests security issues, always indicate that security analysis is needed.
         Be precise and data-driven in your analysis."""),
        ...
        ("security_agent", "AWS security and compliance expert", 
         ...
         IMPORTANT: Your security investigation should proceed in phases:
         - First iteration: Identify immediate threats and security concerns
         - Second iteration: Provide detailed forensic analysis of suspicious activity
         - Third iteration: Develop a comprehensive remediation plan
         
         At the end of your analysis, if the situation requires further investigation, ALWAYS include:
         - "NEXT_STEP: We need to investigate [specific security aspect]" for targeted follow-up
         - "CONTINUE: Additional security analysis required" for broader review
         
         If you identify evidence of unauthorized access or resource creation that could explain billing spikes,
         be sure to explicitly link this to the cost concerns.
        ...
        ("logs_agent", "CloudWatch logs and monitoring expert", 
         ...
         IMPORTANT: Your log analysis should follow this progression:
         - First iteration: Identify key events and timeline
         - Second iteration: Correlate events across systems to establish causality
         - Third iteration: Recommend monitoring improvements to prevent future issues
         
         After your analysis, if you've found evidence that needs deeper investigation, ALWAYS include:
         - "NEXT_MESSAGE: [specific question about the logs]" to direct the next investigation
         - "CONTINUE: Timeline analysis shows [brief summary]" to continue general investigation
         
         If you see logs that correlate with security findings or billing increases, explicitly highlight these connections.
...
def run_test(orchestrator):
    ...
    print("\n" + "=" * 80)
    print("AWS CLOUD MANAGEMENT MULTI-AGENT TEST (MULTI-ITERATION)")
    print("=" * 80)
    
    log(f"🧪 Starting test with thread ID: {thread_id}")
    log("🧪 This test simulates an AWS cost spike investigation scenario requiring multiple analysis phases")
    log("🧪 Multiple specialized agents will analyze the situation concurrently in multiple iterations")
    ...
    log(f"🧪 Total iterations completed: {ITERATION_COUNTERS['current']}")
